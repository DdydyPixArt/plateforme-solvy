...
function ConseillerDashboard({ role, userName, userInitials, onLogout }: DashboardProps) {
  ...
  const kpis = [
    { label: "Mes dossiers actifs", value: "4", sub: "+2 cette semaine", icon: FileText, trend: "up", color: "text-blue-600", bg: "bg-blue-50", border: "border-blue-100" },
    { label: "Documents manquants", value: "4", sub: "1 dossier incomplet", icon: AlertCircle, trend: "warn", color: "text-orange-600", bg: "bg-orange-50", border: "border-orange-100" },
    { label: "En attente d'analyse", value: "2", sub: "Score calculé", icon: Clock, trend: "neutral", color: "text-purple-600", bg: "bg-purple-50", border: "border-purple-100" },
    { label: "Décisions reçues", value: "2", sub: "Ce mois-ci", icon: CheckCircle, trend: "up", color: "text-emerald-600", bg: "bg-emerald-50", border: "border-emerald-100" },
  ];
                ...
                {filtered.map((d, i) => {
                  ...
                  return (
                    <tr key={d.id} className={`border-b border-gray-50 hover:bg-amber-50/40 cursor-pointer transition-colors ${i % 2 === 1 ? "bg-gray-50/30" : ""}`}
                      ...
                      <td className="px-4 py-3"><span className="text-sm font-semibold" style={{ color: txt }}>{fmt(d.demande.montant)}</span></td>
                      <td className="px-4 py-3">
                        {d.score ? (
          ...
          <div className="bg-white rounded-xl border border-gray-200 shadow-sm p-5">
            ...
            <div className="mt-4 space-y-2 pt-3 border-t border-gray-100">
              ...
              <div className="flex justify-between text-xs">
                ...
                <span className="font-semibold" style={{ color: "hsl(43 57% 40%)" }}>684</span>
              </div>
            </div>
          </div>
        </div>
...
function AnalysteDashboard({ role, userName, userInitials, onLogout }: DashboardProps) {
  const [, setLocation] = useLocation();
  const aAnalyser = mockDossiers.filter(d => d.status === "score_calcule" || d.status === "en_analyse");
  const decisions = mockDossiers.filter(d => d.decision !== null);
  const fmt = (n: number) => new Intl.NumberFormat("fr-FR", { style: "currency", currency: "EUR", maximumFractionDigits: 0 }).format(n);
  ...
  const kpis = [
    ...
    { label: "Score moyen", value: "710", sub: "Portefeuille actif", icon: BarChart3, color: "text-purple-600", bg: "bg-purple-50", border: "border-purple-100" },
    { label: "Décisions ce mois", value: String(decisions.length), sub: "Accord: 2 · Refus: 1", icon: CheckCircle, color: "text-emerald-600", bg: "bg-emerald-50", border: "border-emerald-100" },
    { label: "Alertes conformité", value: "2", sub: "1 PPE · 1 FICP", icon: AlertCircle, color: "text-red-600", bg: "bg-red-50", border: "border-red-100" },
  ];
      ...
      <div className="flex-1 overflow-y-auto px-8 py-6 space-y-6">
                ...
                return (
                  <div key={d.id} onClick={() => setLocation("/analyste")}
                    ...
                    <div className="flex-1">
                      ...
                      <div className="text-xs mt-0.5" style={{ color: sub }}>{fmt(d.demande.montant)} · {d.situationPro.statut}</div>
                    </div>
                ...
                return (
                  <div key={d.id} className="flex items-center justify-between px-5 py-3.5">
                    <div>
                      <span className="text-xs font-mono font-semibold" style={{ color: "hsl(43 57% 38%)" }}>{d.reference}</span>
                      <div className="text-sm font-medium mt-0.5" style={{ color: txt }}>{d.client.prenom} {d.client.nom}</div>
                      <div className="text-xs mt-0.5" style={{ color: sub }}>{fmt(d.demande.montant)}</div>
                    </div>
        ...
        <div className="bg-white rounded-xl border border-gray-200 shadow-sm p-5">
          ...
          <ResponsiveContainer width="100%" height={100}>
            <BarChart data={[
              { range: "0–200", count: 0 }, { range: "200–400", count: 1 },
              { range: "400–600", count: 0 }, { range: "600–800", count: 2 }, { range: "800–1000", count: 1 },
            ]} barSize={28}>
...
function AdminDashboard({ role, userName, userInitials, onLogout }: DashboardProps) {
  ...
  const kpis = [
    ...
    { label: "Dossiers total", value: "47", sub: "Ce trimestre", icon: FileText, color: "text-purple-600", bg: "bg-purple-50", border: "border-purple-100" },
    { label: "Taux d'accord", value: "68%", sub: "+4pts vs. mois dernier", icon: TrendingUp, color: "text-emerald-600", bg: "bg-emerald-50", border: "border-emerald-100" },
