...
import { CheckCircle, XCircle, AlertTriangle, Upload, MessageSquare, ArrowLeft, CheckCheck, RotateCcw } from "lucide-react";
...
export default function DossiersIncomplets({ role, userName, userInitials, onLogout }: Props) {
  ...
  const incomplets = mockDossiers.filter(d => d.status === "incomplet");

  const [selected, setSelected] = useState(incomplets[0]);
  const [docStatuts, setDocStatuts] = useState<DocStatut[]>(selected?.documents.map(d => d.statut) || []);
  const [commentaires, setCommentaires] = useState<string[]>(selected?.documents.map(() => "") || []);
  const [showComment, setShowComment] = useState<boolean[]>(selected?.documents.map(() => false) || []);
  ...
  const markReceived = (idx: number) => {
    setDocStatuts(prev => { const n = [...prev]; n[idx] = "fourni"; return n; });
  };
  ...
  return (
    <Layout role={role} userName={userName} userInitials={userInitials} onLogout={onLogout}>
      ...
      <div className="flex-1 overflow-hidden flex">
        {/* Left: list of incomplete dossiers */}
        <div className="w-72 flex-shrink-0 bg-white border-r border-gray-200 overflow-y-auto">
          ...
          <div className="p-3 space-y-2">
            {incomplets.map(d => {
              ...
              const f = docs.filter(doc => doc.statut === "fourni").length;
              const pct = Math.round((f / docs.length) * 100);
              const isSelected = selected?.id === d.id;
              return (
                <button key={d.id} onClick={() => {
                  setSelected(d);
                  setDocStatuts(d.documents.map(doc => doc.statut));
          ...
          <div className="flex-1 overflow-y-auto px-6 py-5 space-y-5">
            ...
            <div className="space-y-3">
              ...
              {selected.documents.map((doc, i) => {
                const statut = docStatuts[i] || doc.statut;
                  ...
                  <div key={i} className="bg-white rounded-xl border border-gray-200 shadow-sm overflow-hidden">
                    <div className="flex items-center gap-4 p-4">
                      ...
                      <div className="flex items-center gap-2 flex-shrink-0">
                        ...
                        {statut === "fourni" && (
                          <button onClick={() => { const n = [...docStatuts]; n[i] = "a_verifier"; setDocStatuts(n); }}
                    ...
                    {showComment[i] && (
                      <div className="px-4 pb-4 pt-0 border-t border-gray-100">
                        <textarea
                          ...
                          placeholder="Ajouter une note ou un commentaire sur ce document..."
                          rows={2}
            ...
            <div className="flex justify-end gap-3 pt-2">
              ...
              <button onClick={handleSave}
                ...
                {saved ? <><CheckCircle className="w-4 h-4" /> Sauvegardé</> : "Enregistrer les modifications"}
              </button>
            </div>
          </div>
        )}
