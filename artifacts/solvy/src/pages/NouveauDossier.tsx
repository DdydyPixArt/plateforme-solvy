...
export default function NouveauDossier({ role, userName, userInitials, onLogout }: NouveauDossierProps) {
  ...
  const Field = ({ label, type = "text", value, onChange, placeholder, options }: any) => (
    <div>
      ...
      {options ? (
        <select value={value} onChange={onChange} className="w-full bg-background border border-border rounded-lg px-3 py-2.5 text-sm text-foreground focus:outline-none focus:border-primary/60">
          {options.map((o: string) => <option key={o}>{o}</option>)}
        </select>
      ) : (
        <input type={type} value={value} onChange={onChange} placeholder={placeholder}
          className="w-full bg-background border border-border rounded-lg px-3 py-2.5 text-sm text-foreground placeholder:text-muted-foreground focus:outline-none focus:border-primary/60 focus:ring-1 focus:ring-primary/20 transition-all" />
      )}
    </div>
  );
      ...
      <Layout role={role} userName={userName} userInitials={userInitials} onLogout={onLogout}>
        <div className="flex-1 flex items-center justify-center">
          <div className="text-center">
            ...
            <h2 className="text-xl font-semibold text-foreground mb-2">Dossier soumis avec succès</h2>
  ...
  return (
    <Layout role={role} userName={userName} userInitials={userInitials} onLogout={onLogout}>
      <PageHeader title="Nouveau dossier de crédit" subtitle="Saisie des informations client" />
            ...
            return (
              <div key={step.id} className="flex items-center flex-1 last:flex-none">
                <div className="flex flex-col items-center gap-1.5">
                  <div className={`w-9 h-9 rounded-full border-2 flex items-center justify-center transition-all ${done ? "border-primary bg-primary" : active ? "border-primary bg-primary/10" : "border-border bg-background"}`}>
                    {done ? <Check className="w-4 h-4 text-primary-foreground" /> : <Icon className={`w-4 h-4 ${active ? "text-primary" : "text-muted-foreground"}`} />}
                  </div>
                  <span className={`text-[10px] font-medium whitespace-nowrap ${active ? "text-primary" : done ? "text-foreground" : "text-muted-foreground"}`}>{step.label}</span>
                </div>
                {i < steps.length - 1 && (
                  <div className={`flex-1 h-0.5 mx-3 mb-4 transition-all ${done ? "bg-primary" : "bg-border"}`} />
                )}
              </div>
            );
          })}
        ...
        <div className="max-w-2xl mx-auto">
          <div className="bg-card border border-border rounded-xl p-6">
            ...
            {currentStep === 1 && (
              <div className="space-y-4">
                <h3 className="text-base font-semibold text-foreground mb-4">Identité du client</h3>
            ...
            {currentStep === 2 && (
              <div className="space-y-4">
                <h3 className="text-base font-semibold text-foreground mb-4">Situation professionnelle</h3>
                <div className="grid grid-cols-2 gap-4">
            ...
            {currentStep === 3 && (
              <div className="space-y-4">
                <h3 className="text-base font-semibold text-foreground mb-4">Revenus et charges</h3>
                <div className="grid grid-cols-2 gap-4">
                ...
                {form.revenusNets && form.chargesFixes && (
                  <div className="mt-4 p-4 rounded-xl bg-muted/30 border border-border space-y-2">
                    <div className="text-xs font-medium text-muted-foreground uppercase tracking-wider mb-2">Calcul préliminaire</div>
                    <div className="flex justify-between text-sm">
                      <span className="text-muted-foreground">Taux d'endettement estimé</span>
                      <span className={`font-semibold ${((Number(form.chargesFixes) + Number(form.creditsEnCours)) / Number(form.revenusNets)) * 100 > 35 ? "text-red-400" : "text-green-400"}`}>
                        {(((Number(form.chargesFixes) + Number(form.creditsEnCours)) / Number(form.revenusNets)) * 100).toFixed(1)}%
                      </span>
                    </div>
                    <div className="flex justify-between text-sm">
                      <span className="text-muted-foreground">Reste à vivre estimé</span>
                      <span className="font-semibold text-foreground">{(Number(form.revenusNets) + Number(form.autresRevenus || 0) - Number(form.chargesFixes) - Number(form.creditsEnCours || 0)).toLocaleString("fr-FR")} €/mois</span>
                    </div>
                  </div>
                )}
              </div>
            ...
            {currentStep === 4 && (
              <div className="space-y-4">
                <h3 className="text-base font-semibold text-foreground mb-4">Demande de crédit</h3>
                <div className="grid grid-cols-2 gap-4">
            ...
            {currentStep === 5 && (
              <div className="space-y-4">
                <h3 className="text-base font-semibold text-foreground mb-2">Documents justificatifs</h3>
                <p className="text-xs text-muted-foreground mb-4">Cliquer sur le statut pour le faire évoluer</p>
                <div className="space-y-2">
                  {docs.map((doc, i) => {
                    const Icon = doc.statut === "fourni" ? CheckCircle2 : doc.statut === "manquant" ? XCircle : AlertCircle;
                    const colors = { fourni: "text-green-400 bg-green-400/10 border-green-400/20", manquant: "text-red-400 bg-red-400/10 border-red-400/20", a_verifier: "text-yellow-400 bg-yellow-400/10 border-yellow-400/20" };
                    ...
                    return (
                      <div key={i} className="flex items-center justify-between p-3 rounded-lg bg-background border border-border">
                        <div className="flex items-center gap-3">
                          <Icon className={`w-4 h-4 flex-shrink-0 ${doc.statut === "fourni" ? "text-green-400" : doc.statut === "manquant" ? "text-red-400" : "text-yellow-400"}`} />
                          <span className="text-sm text-foreground">{doc.nom}</span>
                        </div>
                        <button onClick={() => cycleDocStatus(i)} className={`text-xs font-medium px-3 py-1 rounded-full border cursor-pointer transition-all ${colors[doc.statut]}`}>
                          {labels[doc.statut]}
                        </button>
                      </div>
                    );
                ...
                <div className="flex gap-4 mt-2 text-xs text-muted-foreground pt-2 border-t border-border">
                  <span className="text-green-400">{docs.filter(d => d.statut === "fourni").length} fournis</span>
                  <span className="text-red-400">{docs.filter(d => d.statut === "manquant").length} manquants</span>
                  <span className="text-yellow-400">{docs.filter(d => d.statut === "a_verifier").length} à vérifier</span>
                </div>
              </div>
            )}
          </div>
          ...
          <div className="flex justify-between mt-6">
            <button onClick={() => currentStep > 1 ? setCurrentStep(s => s - 1) : setLocation("/dashboard")}
              className="flex items-center gap-2 px-5 py-2.5 rounded-lg border border-border text-sm text-muted-foreground hover:text-foreground hover:border-border/80 transition-all">
              <ChevronLeft className="w-4 h-4" />
              {currentStep > 1 ? "Précédent" : "Annuler"}
            </button>
            {currentStep < 5 ? (
              <button onClick={() => setCurrentStep(s => s + 1)}
                className="flex items-center gap-2 bg-primary hover:bg-primary/90 text-primary-foreground text-sm font-medium px-5 py-2.5 rounded-lg transition-all">
